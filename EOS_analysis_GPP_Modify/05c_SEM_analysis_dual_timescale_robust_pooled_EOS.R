...
DATA_DIR <- file.path(OUTPUT_ROOT, "SEM_Data_Dual_Fixed_Robust_Pooled_EOS")
OUTPUT_DIR <- file.path(OUTPUT_ROOT, "SEM_Results_Dual_Fixed_Robust_Pooled_EOS")
...
check_raster_alignment <- function(template_r, year) {
  sample_files <- list(
    ...
    list(file.path(PHENO_DIR, sprintf("eos_gpp_%d.tif", year)), "EOS", FALSE),
    list(file.path(PHENO_DIR, sprintf("pos_doy_gpp_%d.tif", year)), "POS", FALSE),
    list(file.path(DERIVED_DIR, sprintf("Fixed_GPPrate_%d.tif", year)), "Fixed_GPPrate", TRUE),
...
eos_test_r <- raster(file.path(PHENO_DIR, sprintf("eos_gpp_%d.tif", year_test)))
gpp_test_r <- raster(file.path(DERIVED_DIR, sprintf("Fixed_GPPrate_%d.tif", year_test)))
...
compareRaster(
  fixed_len_r, tr_test_r, eos_test_r, gpp_test_r,
...
cat(sprintf("      EOS:        %6d (%.2f%%)\n", sum(!is.na(eos_test)), 100*sum(!is.na(eos_test))/n_cells_total))
...
# 核心3变量交集（类似05b）
valid_core <- !is.na(tr_test) & !is.na(eos_test) & !is.na(gpp_test) &
...
valid_1982 <- !is.na(tr_test) & !is.na(eos_test) & !is.na(gpp_test) &
...
# 清理测试数据
rm(tr_test_r, eos_test_r, gpp_test_r, p_pre_test_r, t_pre_test_r,
   sw_pre_test_r, p_season_test_r, t_season_test_r, sw_season_test_r,
   tr_test, eos_test, gpp_test, p_pre_test, t_pre_test, sw_pre_test,
...
read_year_data <- function(year, candidate_cells, fixed_len_vals_all, fixed_len_nodata) {
  ...
  eos_r <- raster(file.path(PHENO_DIR, sprintf("eos_gpp_%d.tif", year)))
  gpp_season_r <- raster(file.path(DERIVED_DIR, sprintf("Fixed_GPPrate_%d.tif", year)))
...
# 统计每个变量的基本信息
cat("\n  变量基本统计:\n")
vars_to_summarize <- c("Fixed_Trate", "EOS", "Fixed_GPPrate",
                       "P_pre", "T_pre", "SW_pre",
                       "P_season", "T_season", "SW_season")
...
safe_write_csv(long_df_std, file.path(DATA_DIR, "sem_dual_timescale_standardized.csv"), row.names = FALSE)
  ...
  apply_indirect_delta_p <- function(params_df) {
    ...
    p_pre_via_eos_gpp <- delta_p_three(a1$std, b$std, d$std, a1$se_std, b$se_std, d$se_std)
    t_pre_via_eos_gpp <- delta_p_three(a2$std, b$std, d$std, a2$se_std, b$se_std, d$se_std)
    sw_pre_via_eos_gpp <- delta_p_three(a3$std, b$std, d$std, a3$se_std, b$se_std, d$se_std)
    p_pre_via_eos <- delta_p_two(a1$std, g$std, a1$se_std, g$se_std)
    t_pre_via_eos <- delta_p_two(a2$std, g$std, a2$se_std, g$se_std)
    sw_pre_via_eos <- delta_p_two(a3$std, g$std, a3$se_std, g$se_std)
    ...
    p_season_via_eos_gpp <- delta_p_three(a4$std, b$std, d$std, a4$se_std, b$se_std, d$se_std)
    t_season_via_eos_gpp <- delta_p_three(a5$std, b$std, d$std, a5$se_std, b$se_std, d$se_std)
    sw_season_via_eos_gpp <- delta_p_three(a6$std, b$std, d$std, a6$se_std, b$se_std, d$se_std)
    p_season_via_eos <- delta_p_two(a4$std, g$std, a4$se_std, g$se_std)
    t_season_via_eos <- delta_p_two(a5$std, g$std, a5$se_std, g$se_std)
    sw_season_via_eos <- delta_p_two(a6$std, g$std, a6$se_std, g$se_std)
    ...
    eos_via_gpp <- delta_p_two(b$std, d$std, b$se_std, d$se_std)
    ...
    p_season_indirect <- a4$std * b$std * d$std + a4$std * g$std + c1$std * d$std
    t_season_indirect <- a5$std * b$std * d$std + a5$std * g$std + c2$std * d$std
    sw_season_indirect <- a6$std * b$std * d$std + a6$std * g$std + c3$std * d$std
    ...
    p_season_indirect_p <- {
      var_sum <- sum(c(
        delta_var_three(a4$std, b$std, d$std, a4$se_std, b$se_std, d$se_std),
        delta_var_two(a4$std, g$std, a4$se_std, g$se_std),
        delta_var_two(c1$std, d$std, c1$se_std, d$se_std)
      ), na.rm = TRUE)
      if (is.finite(var_sum) && var_sum > 0) {
    ...
    t_season_indirect_p <- {
      var_sum <- sum(c(
        delta_var_three(a5$std, b$std, d$std, a5$se_std, b$se_std, d$se_std),
        delta_var_two(a5$std, g$std, a5$se_std, g$se_std),
        delta_var_two(c2$std, d$std, c2$se_std, d$se_std)
      ), na.rm = TRUE)
      if (is.finite(var_sum) && var_sum > 0) {
    ...
    sw_season_indirect_p <- {
      var_sum <- sum(c(
        delta_var_three(a6$std, b$std, d$std, a6$se_std, b$se_std, d$se_std),
        delta_var_two(a6$std, g$std, a6$se_std, g$se_std),
        delta_var_two(c3$std, d$std, c3$se_std, d$se_std)
      ), na.rm = TRUE)
      if (is.finite(var_sum) && var_sum > 0) {
    ...
    params_df <- update_defined("P_pre_via_EOS_GPP", p_pre_via_eos_gpp)
    params_df <- update_defined("T_pre_via_EOS_GPP", t_pre_via_eos_gpp)
    params_df <- update_defined("SW_pre_via_EOS_GPP", sw_pre_via_eos_gpp)
    params_df <- update_defined("P_pre_via_EOS", p_pre_via_eos)
    params_df <- update_defined("T_pre_via_EOS", t_pre_via_eos)
    params_df <- update_defined("SW_pre_via_EOS", sw_pre_via_eos)
    ...
    params_df <- update_defined("P_season_via_EOS_GPP", p_season_via_eos_gpp)
    params_df <- update_defined("T_season_via_EOS_GPP", t_season_via_eos_gpp)
    params_df <- update_defined("SW_season_via_EOS_GPP", sw_season_via_eos_gpp)
    params_df <- update_defined("P_season_via_EOS", p_season_via_eos)
    params_df <- update_defined("T_season_via_EOS", t_season_via_eos)
    params_df <- update_defined("SW_season_via_EOS", sw_season_via_eos)
    ...
    params_df <- update_defined("EOS_via_GPP", eos_via_gpp)
    ...
    params_df <- update_defined("P_season_indirect", p_season_indirect_p)
    params_df <- update_defined("T_season_indirect", t_season_indirect_p)
    params_df <- update_defined("SW_season_indirect", sw_season_indirect_p)
  ...
  cat("\n模型结构:\n")
  cat("  第一层: 早/晚季气候 → EOS\n")
  cat("  第二层: EOS + 早/晚季气候 → GPP\n")
  cat("  第三层: EOS + GPP + 早/晚季气候 → Fixed_Trate\n")
  cat("\n  包含完整的直接效应和间接效应路径\n")
...
safe_write_csv(boot_mat, file.path(OUTPUT_DIR, "SEM_dual_timescale_bootstrap_coefficients.csv"), row.names = FALSE)
  ...
  key_paths <- c("a1", "a2", "a3", "a4", "a5", "a6",  # 早/晚季 → EOS
                 "b", "f1", "f2", "f3", "c1", "c2", "c3",  # → GPP
                 "g", "d", "h1", "h2", "h3", "e1", "e2", "e3",  # → Fixed_Trate
                 "P_pre_via_EOS_GPP", "T_pre_via_EOS_GPP", "SW_pre_via_EOS_GPP",
                 "P_pre_via_EOS", "T_pre_via_EOS", "SW_pre_via_EOS",
  ...
  report <- c(
    ...
    "模型结构:",
    "----------------------------------------------------------------------",
    "第一层: 早季气候(P_pre, T_pre, SW_pre) + 晚季气候(P_season, T_season, SW_season) → EOS",
    "第二层: EOS + 早/晚季气候 → GPP",
    "第三层: EOS + GPP + 早/晚季气候 → Fixed_Trate",
    "",
    "----------------------------------------------------------------------",
    "模型拟合指标:",
    "----------------------------------------------------------------------",
