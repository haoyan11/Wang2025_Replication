...
check_raster_alignment <- function(template_r, year) {
  sample_files <- list(
    ...
    list(file.path(PHENO_DIR, sprintf("eos_ndvi_%d.tif", year)), "EOS", FALSE),
...
eos_test_r <- raster(file.path(PHENO_DIR, sprintf("eos_ndvi_%d.tif", year_test)))
...
read_year_data <- function(year, candidate_cells, fixed_len_vals_all, fixed_len_nodata) {
  ...
  eos_r <- raster(file.path(PHENO_DIR, sprintf("eos_ndvi_%d.tif", year)))
