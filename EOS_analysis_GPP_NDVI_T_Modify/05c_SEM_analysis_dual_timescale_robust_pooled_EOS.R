...
# ===== 05c模块专用输出目录 =====
DATA_DIR <- file.path(MODE_ROOT, "SEM_Data_Dual_Fixed_Robust_Pooled_EOS")
OUTPUT_DIR <- file.path(MODE_ROOT, "SEM_Results_Dual_Fixed_Robust_Pooled_EOS")
...
check_raster_alignment <- function(template_r, year) {
  sample_files <- list(
    ...
    list(file.path(PHENO_DIR, sprintf(EOS_PATTERN, year)), "EOS", FALSE),
...
for (year in years) {
  ...
  # 检查EOS（物候数据）
  eos_pattern <- sprintf("eos_%s_%%d.tif", tolower(MIDDLE_VAR_NAME))
  eos_file <- file.path(PHENO_DIR, sprintf(eos_pattern, year))
...
eos_test_r <- raster(file.path(PHENO_DIR, sprintf(EOS_PATTERN, year_test)))
...
read_year_data <- function(year, candidate_cells, fixed_len_vals_all, fixed_len_nodata) {
  ...
  eos_r <- raster(file.path(PHENO_DIR, sprintf(EOS_PATTERN, year)))
  gpp_season_r <- raster(file.path(DECOMP_DIR, sprintf(FIXED_RATE_PATTERN, year)))
