...
def calculate_GPPc_block_optimized(year, mask, template_profile):
    ...
    valid_mask &= np.isfinite(pos) & np.isfinite(eos)
    if pos_nodata is not None:
        valid_mask &= (pos != pos_nodata)
    if eos_nodata is not None:
        valid_mask &= (eos != eos_nodata)
    valid_mask &= (pos > 0) & (pos <= 365)
    valid_mask &= (eos > 0) & (eos <= 365)
    valid_mask &= (pos < eos)

    pos = np.where(valid_mask, pos, 0).astype(np.int16)
    eos = np.where(valid_mask, eos, 0).astype(np.int16)

    # 找到所有可能的DOY范围
    min_doy = int(np.min(pos[valid_mask])) if np.any(valid_mask) else 1
    max_doy = int(np.max(eos[valid_mask])) if np.any(valid_mask) else 365
    ...
    for doy in tqdm(range(min_doy, max_doy + 1), desc=f"GPPc {year}", leave=False):
        ...
        # 找到需要累加的像元（POS <= DOY <= EOS）
        to_add = valid_mask & (pos <= doy) & (doy <= eos)
...
def save_climatology_data_GPP():
    ...
    - GPPc_av.tif : 基于[POSav, EOSav]的多年平均累积GPP (gC/m²)
    """
    print("\n" + "="*70)
    print("计算GPP气候态数据（EOS版本）")
    print("="*70)
...
if __name__ == "__main__":
    ...
    if RUN_MODE == "skip" and outputs_complete(years):
        ...
        #   - GPPc_av.tif (基于POSav和EOSav的多年平均累积GPP)
        #
        # 输出目录：Wang2025_Analysis_EOS_GPP_Modify/Climatology/
        #
