...
#
# Purpose:
# - Keep the 05b model structure (SOS -> GPP -> Fixed_Trate).
...
sem_model <- '
  ...
  # 第二层：SOS + 季前气候 + 生长季气候 → Fixed_GPPrate（碳固定）
  Fixed_GPPrate ~ b*SOS + f1*P_pre + f2*T_pre + f3*SW_pre +
               c1*P_season + c2*T_season + c3*SW_season

  # 第三层：SOS + Fixed_GPPrate + 季前气候 + 生长季气候 → Fixed_Trate（固定窗口蒸腾速率）
...
r2_names <- c("SOS", "Fixed_GPPrate", "Fixed_Trate")
...
extract_lavaan_results <- function(df_std) {
  ...
  p_via_sos_gpp <- a1 * b * d
  t_via_sos_gpp <- a2 * b * d
  sw_via_sos_gpp <- a3 * b * d
  p_via_sos <- a1 * g
  t_via_sos <- a2 * g
  sw_via_sos <- a3 * g
  p_via_gpp <- f1 * d
  ...
  p_p_via_sos_gpp <- delta_p_three(a1_stats$std, b_stats$std, d_stats$std,
                                   a1_stats$se_std, b_stats$se_std, d_stats$se_std)
  p_t_via_sos_gpp <- delta_p_three(a2_stats$std, b_stats$std, d_stats$std,
                                   a2_stats$se_std, b_stats$se_std, d_stats$se_std)
  p_sw_via_sos_gpp <- delta_p_three(a3_stats$std, b_stats$std, d_stats$std,
                                    a3_stats$se_std, b_stats$se_std, d_stats$se_std)
  p_p_via_sos <- delta_p_two(a1_stats$std, g_stats$std, a1_stats$se_std, g_stats$se_std)
  p_t_via_sos <- delta_p_two(a2_stats$std, g_stats$std, a2_stats$se_std, g_stats$se_std)
  p_sw_via_sos <- delta_p_two(a3_stats$std, g_stats$std, a3_stats$se_std, g_stats$se_std)
  ...
  p_sos_via_gpp <- delta_p_two(b_stats$std, d_stats$std, b_stats$se_std, d_stats$se_std)
  ...
  vals <- c(
    ...
    p_via_sos_gpp, t_via_sos_gpp, sw_via_sos_gpp,
    p_via_sos, t_via_sos, sw_via_sos,
  ...
  p_vals <- c(
    ...
    p_p_via_sos_gpp, p_t_via_sos_gpp, p_sw_via_sos_gpp,
    p_p_via_sos, p_t_via_sos, p_sw_via_sos,
...
run_pixel_sem_lavaan_compare <- function(years, mask_r, fixed_window_length_r,
  ...
  files <- list(
    ...
    SOS = file.path(PHENO_DIR, sprintf("sos_ndvi_%d.tif", years)),
...
main <- function() {
  ...
  for (year in years) {
    files_to_check <- c(
      ...
      file.path(PHENO_DIR, sprintf("sos_ndvi_%d.tif", year)),
  ...
  sample_rasters <- list(
    ...
    raster(file.path(PHENO_DIR, sprintf("sos_ndvi_%d.tif", YEAR_START))),
