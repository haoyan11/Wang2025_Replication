...
from _config import (
    ...
    MAX_WORKERS, TEMPLATE_RASTER, MASK_FILE, VAR_DAILY_DIR, VAR_DAILY_FORMAT,
    OUTPUT_CUMULATIVE_FORMAT, OUTPUT_CLIMATOLOGY_FORMAT, OUTPUT_DECOMP_FORMAT,
    MIDDLE_VAR_NAME,  # 用于日志标签
    PHENO_FILE_FORMAT  # 物候文件命名格式
)
...
def _process_year(year):
    if not OVERWRITE:
        outputs = [
            ...
            OUTPUT_DIR / OUTPUT_DECOMP_FORMAT['sos_change'].format(year=year),
        ...
        if all(p.exists() for p in outputs):
            return year
    ctx = _GLOBAL_CTX
    (tr_window_change, tr_fixed_window, tr_sos_change, tr_pos_change,
     gpp_window_change, gpp_fixed_window, gpp_sos_change, gpp_pos_change) = decompose_year(
        year,
        ctx["trc_av"],
        ctx["sos_av"],
        ctx["pos_av"],
    ...
    write_geotiff(OUTPUT_DIR / OUTPUT_DECOMP_FORMAT['sos_change'].format(year=year), gpp_sos_change, ctx["profile"])
...
def accumulate_daily_gpp_sums(year, ref_profile, valid_base,
    ...
    返回:
      gppc_y_actual, gppc_y_fixed_actual, gpp_sos_advance, gpp_sos_delay,
      gpp_pos_extend, gpp_pos_shorten, gpp_valid_y, gppc_y_count, gppc_y_fixed_count
    """
    ...
    gpp_sos_advance = np.zeros((height, width), dtype=np.float32)
    gpp_sos_delay = np.zeros((height, width), dtype=np.float32)
    gpp_pos_extend = np.zeros((height, width), dtype=np.float32)
    gpp_pos_shorten = np.zeros((height, width), dtype=np.float32)
        ...
        for day in range(1, days_in_year + 1):
            ...
            with rasterio.open(file_path) as src:
                ...
                for win, has_valid in zip(block_windows, block_has_valid):
                    ...
                    data[data < 0] = np.nan  # NDVI负值过滤，与其他模块保持一致
                    ...
                    m_gppc = valid_day & (sos_y_block <= doy) & (pos_y_block >= doy)
                    if np.any(m_gppc):
                    ...
                    if np.any(m_sos_adv):
                        sos_adv_block = gpp_sos_advance[r0:r1, c0:c1]
                    ...
                    if np.any(m_sos_delay):
                        sos_delay_block = gpp_sos_delay[r0:r1, c0:c1]
                    ...
                    if np.any(m_pos_extend):
                        pos_ext_block = gpp_pos_extend[r0:r1, c0:c1]
                    ...
                    if np.any(m_pos_shorten):
                        pos_short_block = gpp_pos_shorten[r0:r1, c0:c1]
                        pos_short_block[m_pos_shorten] += data[m_pos_shorten]

    return (gppc_y_actual, gppc_y_fixed_actual, gpp_sos_advance, gpp_sos_delay,
            gpp_pos_extend, gpp_pos_shorten, gpp_valid_y, gppc_y_count, gppc_y_fixed_count)
...
def decompose_year(year, trc_av, sos_av, pos_av,
    ...
    gpp_sos_change : GPP SOS变化的贡献
    gpp_pos_change : GPP POS变化的贡献
    """
    # 读取当年数据
    trc_file = TRC_DIR / f"TRc_{year}.tif"
    gppc_file = TRC_DIR / OUTPUT_CUMULATIVE_FORMAT.format(year=year)  # NDVIc也在TRc_annual目录
    ...
    check_spatial_consistency(ref_profile, sos_file, sos_profile, PHENO_FILE_FORMAT['SOS'].format(year=year))
    ...
    gpp_sos_change = np.full((height, width), NODATA_OUT, dtype=np.float32)
    ...
    if not np.any(valid_base):
        return (tr_window_change, tr_fixed_window, tr_sos_change, tr_pos_change,
                gpp_window_change, gpp_fixed_window, gpp_sos_change, gpp_pos_change)
    ...
    # GPP分解
    (gppc_y_actual, gppc_y_fixed_actual, gpp_sos_advance, gpp_sos_delay,
     gpp_pos_extend, gpp_pos_shorten, gpp_valid_y, gppc_y_count, gppc_y_fixed_count) = accumulate_daily_gpp_sums(
        year, ref_profile, valid_base, sos_y_i, pos_y_i, sos_av_i, pos_av_i
    )

    valid_sos = valid_sos & tr_valid_y & gpp_valid_y
    ...
    #    GPP_fixed_window = sum_{SOSav..POSav} [G_y(t) - G_av(t)]
    gpp_fixed_window[valid_sos] = (gppc_y_fixed_actual - gppc_av)[valid_sos]

    # 4. 窗口变化部分（残差）
    #    TR_window_change = TRc_y - sum_{SOSav..POSav} B_y(t)
    tr_window_change[valid_sos] = (trc_y_actual - trc_y_fixed_actual)[valid_sos]
    #    GPP_window_change = GPPc_y - sum_{SOSav..POSav} G_y(t)
    gpp_window_change[valid_sos] = (gppc_y_actual - gppc_y_fixed_actual)[valid_sos]
    ...
    gpp_sos_change[valid_sos] = np.where(
        sos_before,
        gpp_sos_advance,
        -gpp_sos_delay
    )[valid_sos]
    ...
    gpp_pos_change[valid_sos] = np.where(
        pos_after,
        gpp_pos_extend,
        -gpp_pos_shorten
    )[valid_sos]
...
def main(run_cfg=None):
    ...
    # Load NDVI climatology (NDVIc_av在Climatology目录)
    ...
    # 保存GPP固定窗口长度（与TR相同）
    fixed_gpp_window_length = np.full(sos_av.shape, NODATA_OUT, dtype=np.float32)
    fixed_gpp_window_length[valid_av] = (pos_av_i - sos_av_i + 1)[valid_av]
    ...
    if PARALLEL_BY_YEAR and YEAR_WORKERS > 1:
        ...
        for year in tqdm(years, desc="Fixed-Window Decomposition"):
            ...
            write_geotiff(OUTPUT_DIR / OUTPUT_DECOMP_FORMAT['sos_change'].format(year=year), gpp_sos_change, profile)
    ...
    sos_y, _, nodata_sos_y = read_geotiff(PHENO_DIR / PHENO_FILE_FORMAT['SOS'].format(year=test_year))
    pos_y, _, nodata_pos_y = read_geotiff(PHENO_DIR / PHENO_FILE_FORMAT['POS'].format(year=test_year))
