  ...
  process_block <- function(i) {
    ...
    for (idx in valid_cells) {
      ...
      df_std <- data.frame(
        ...
        GPP_season = scale_vec(df$GPP_season),
...
main <- function() {
  ...
  for (year in years) {
    files_to_check <- c(
      ...
      file.path(DERIVED_DIR, sprintf("GPP_season_%d.tif", year)),
  ...
  sample_rasters <- list(
    ...
    raster(file.path(DERIVED_DIR, sprintf("GPP_season_%d.tif", YEAR_START))),
